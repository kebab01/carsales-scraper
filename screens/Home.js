...
export default function Home({route, navigation}) {
    ...
    const [isRetrieving, changeRetieveStatus] = useState(false)
    ...
    const getExpression = () =>{
        ...
        if (selectBrand != undefined && selectModel != undefined && selectBodyType != undefined && selectState != undefined){
            car = `"(And.(C.Make.${selectBrand}._.Model.${selectModel}.)_.BodyStyle.${selectBodyType}._.State.${STATE_EXPR_ENUM[selectState]}.)"`

        }
        else if (selectBrand != undefined && selectModel != undefined && selectBodyType != undefined){
            ...
            car = `"(C.Make.${selectBrand}._.Model.${selectModel}.)"`
        }
        else if (selectBrand != undefined){
            
            car = `"Make.${selectBrand}."`
        }
        else{
            car = `""`
        }
  ...
  return (
    <SafeAreaView style={{padding:10}}>
        <View style={{width:'100%', alignItems:'center'}}>
            <Text style={{fontSize:20, fontWeight:'bold', paddingTop:20, paddingBottom:20}}>Carsales Comparision</Text>
        </View>

        <TouchableOpacity onPress={() => navigation.navigate('InfoFill', {data:brands, title:'Brand', var:'brand', currentObj:route.params})}>
            <InfoComponent title='Brand:' selected={selectBrand}/>
        </TouchableOpacity>

        <TouchableOpacity onPress={() => navigation.navigate('InfoFill', {data:models, title:'Model', var:'model', currentObj:route.params})}>
            <InfoComponent title='Model:' selected={selectModel}/>
        </TouchableOpacity>

        <TouchableOpacity onPress={() => navigation.navigate('InfoFill', {data:body_types, title:'Body Type', var:'bodyType', currentObj:route.params})}>
            <InfoComponent title='Body Type:' selected={selectBodyType}/>
        </TouchableOpacity>

        <TouchableOpacity onPress={() => navigation.navigate('InfoFill', {data:Object.keys(STATE_ENUMS), title:'State', var:'state', currentObj:route.params})}>
        ...
        <View style={{alignItems:'center', padding:20, height:100}}>
            ...
            <>
                ...
                <Text style={{paddingTop:5}}>{isRetrieving?'Retrieving Data':'Processing data'}</Text>
            </>
            }
        </View>
    </SafeAreaView>
